...
with st.sidebar:
    ...
    st.image("https://tbl.com.bd/frontend/img/products/3.png", width=150)
...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS standalone_indents (
        ...
        # Migration: Add status column if it doesn't exist in older DB versions
    try:
        c.execute("ALTER TABLE standalone_indents ADD COLUMN status TEXT DEFAULT 'Not Purchased'")
    ...
    conn.commit()
    conn.close() # Close connection only at the very end of the function
...
# --- MAIN APPLICATION LOGIC ---
    ...
    if role == 'user':
        menu = ["🔎 View & Filter Requests", "📊 Budget Balance Sheet", "💰 LC/PO & Payment Tracker", "🛒 Indent & Purchase Record"]
    ...
    if page_name == "View & Filter Requests":
        ...
        if requests_df.empty:
            ...
            if st.session_state['show_admin_edit'] and st.session_state['edit_mn_id']:
                ...
                record_to_edit_df = load_data("SELECT * FROM requests WHERE id = ?", (edit_id,))
                ...
                with st.form("admin_edit_form"):
                    ...
                    if save_button:
                        ...
                        if mn_no_new != original_data['mn_number']:
                            conn = sqlite3.connect(DB_FILE)
                            c = conn.cursor()
                            c.execute("SELECT mn_number FROM requests WHERE mn_number = ?", (mn_no_new,))
                            if c.fetchone():
                                conn.close()
    ...
    # --- TAB 2: NEW REQUEST (MN) ---
        ...
        # Guard: User cannot access this
        if st.session_state['role'] not in ['administrator', 'super']:
            st.error("🚫 Access Denied: You do not have permission to create new requests.")
            st.stop()
        st.session_state['show_mn_details'] = False
        st.session_state['show_admin_edit'] = False
        st.session_state['edit_mn_id'] = None
        ...
        with st.form("mn_submission_form"):
            ...
            if submitted:
                ...
                params = (
                    mn_no, date_issue_str, datetime.now().strftime("%Y-%m-%d"), 
                    requester, area, landed_total_cost, "Pending", 
                    mn_particulars, mn_category, selected_department, location, 
                    supplier_vendor, supplier_type, currency, foreign_spare_cost, 
                    freight_fca_charges, customs_duty_pct, local_cost_wo_vat_ait, 
                    vat_ait, landed_total_cost, date_ho_str, plant_remarks
                )
    ...
    # --- TAB 7: EVENT LOG ---
        ...
        with st.form("add_indent_registry_form", clear_on_submit=True):
            ...
            if st.form_submit_button("Add to Indent Registry", type="primary"):
                if reg_indent_no and reg_desc and reg_qty > 0:
                    execute_query('''INSERT INTO standalone_indents 
                                   (indent_number, indent_date, item_description, quantity, unit, rate, total_amount, status) 
                                   VALUES (?, ?, ?, ?, ?, ?, ?, ?)''',
                                   (reg_indent_no, reg_date.strftime("%Y-%m-%d"), reg_desc, reg_qty, reg_unit, reg_rate, reg_qty*reg_rate, "Not Purchased"))
        ...
        # A. Selection Section
        available_indents = load_data("SELECT DISTINCT indent_number FROM standalone_indents WHERE status = 'Not Purchased'")
        selected_indent_no = st.selectbox("Select Indent", ["-- Select Indent --"] + available_indents['indent_number'].tolist())
        
        if selected_indent_no != "-- Select Indent --":
            pending_items = load_data("SELECT indent_id, item_description, quantity, unit, rate FROM standalone_indents WHERE indent_number = ? AND status = 'Not Purchased'", (selected_indent_no,))
            item_options = {f"{r['item_description']} (Avail: {r['quantity']})": r['indent_id'] for _, r in pending_items.iterrows()}
            selected_labels = st.multiselect("Select Items of this Indent", options=list(item_options.keys()))

            # B. Itemized Quantities & Rates (Revision Section)
            if selected_labels:
                st.subheader("📝 Revise Quantities & Rates")
                selected_ids = [item_options[label] for label in selected_labels]
                items_to_process = pending_items[pending_items['indent_id'].isin(selected_ids)]
                
                revised_data = []
                for _, row in items_to_process.iterrows():
                    r_col1, r_col2, r_col3 = st.columns([3, 1, 1])
                    r_col1.write(f"**Item:** {row['item_description']}")
                    new_qty = r_col2.number_input(f"Qty", value=float(row['quantity']), key=f"q_{row['indent_id']}")
                    new_rate = r_col3.number_input(f"Rate", value=float(row['rate']), key=f"r_{row['indent_id']}")
                    revised_data.append({
                        'indent_id': row['indent_id'], 'desc': row['item_description'],
                        'qty': new_qty, 'unit': row['unit'], 'rate': new_rate, 'amount': new_qty * new_rate
                    })

                if st.button("Update Items to Bill"):
                    st.session_state['temp_bill_items'] = revised_data
                    st.success("Items added to temporary display below.")
        ...
        if st.session_state['temp_bill_items']:
            ...
            # D. Billing Information Input Section
            st.subheader("💳 Billing Information")
            with st.form("final_bill_form"):
                ...
                with fcol2:
                    ...
                    pay_mode = st.selectbox("Payment Mode", ["Cash", "Cheque", "EFT", "LC"])
                with fcol3:
                    grn = st.text_input("GRN Number")
                    remarks = st.text_area("Remarks")

                if st.form_submit_button("Save Final Purchase Record"):
                    # Duplicate check
                    dup = load_data("SELECT bill_no FROM indent_purchase_record WHERE bill_no = ?", (bill_no,))
                    if not dup.empty:
                        st.error(f"Error: Bill Number {bill_no} already exists.")
                    elif not bill_no or not supplier:
                        st.error("Please fill mandatory fields (Bill No & Supplier).")
                    else:
                        # Save Header
                        execute_query('''INSERT INTO indent_purchase_record 
                                       (bill_no, indent_no, grn_no, supplier, bill_date, payment_mode, total_bill_amount, remarks) 
                                       VALUES (?, ?, ?, ?, ?, ?, ?, ?)''',
                                       (bill_no, selected_indent_no, grn, supplier, bill_date.strftime("%Y-%m-%d"), pay_mode, total_val, remarks))
                        # Save Details & Update Registry
                        for item in st.session_state['temp_bill_items']:
                            execute_query("INSERT INTO indent_goods_details (bill_no, description, quantity, unit, rate, amount) VALUES (?,?,?,?,?,?)",
                                          (bill_no, item['desc'], item['qty'], item['unit'], item['rate'], item['amount']))
                            execute_query("UPDATE standalone_indents SET status = 'Billed' WHERE indent_id = ?", (item['indent_id'],))
                        
                        st.session_state['temp_bill_items'] = [] # Clear temp list
                        st.success(f"Bill {bill_no} recorded successfully!")
                        st.rerun()
